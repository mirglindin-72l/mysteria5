...
proc delta {f t} {
	...
	###
	log_puts "ALL" "delta ###"
	log_puts "ALL" "delta Source from:"
	log_puts "ALL" $f
	log_puts "ALL" "delta ###"
	log_puts "ALL" "delta Source to:"
	log_puts "ALL" $t
	log_puts "ALL" "delta ###"
	log_puts "ALL" "delta Diff: "
	set fc 0
	set tc 0
	foreach {k v} [lsort -increasing -integer -stride 2 -index 0 [array get clear_unique]] {
		...
		set del 0
		set add 0
		foreach line [lrange $flines $fc $p_fs-1] {
			append ldel "-$line\n"
			incr del 1
		}
		foreach line [lrange $tlines $tc $p_ts-1] {
			append ladd "+$line\n"
			incr add 1
		}
		if { $del > 0 } {
			append ret "d $del f $fc t $tc\n"
			append ret $ldel
		}
		if { $add > 0 } {
			append ret "a $add f $fc t $tc\n"
			append ret $ladd
		}
		set fc [expr {$p_fs+$p_l}]
		set tc [expr {$p_ts+$p_l}]
	}
	log_puts "ALL" "delta $ret"
	log_puts "ALL" "delta ###"
	return $ret
}
...
proc apply_delta {f p} {
	...
	while 1 {
		...
		log_puts "ALL" "apply_delta cmd $cmd"
		...
		"" {
			log_puts "ALL" "apply_delta continue"
			continue
		}
		default {
			log_puts "ERR" "apply_delta mode $mode , break"
			break
		}
		}
		while { $fs < $tfs } {
		...
		for { set c 0 } { $c < $num } { incr c 1 } {
			...
			if { $mode == "d" && $p == "-" } {
				if { "$data" != "[lindex $flines $fs]" } {
					log_puts "ERR" "apply_delta delete $data != [lindex $flines $fs]"
				}
				incr fs 1
			} elseif { $mode == "a" && $p == "+" } {
				lappend tlines $data
			} else {
				log_puts "ERR" "apply_delta error $cmd"
			}
		}
	}
	set ret [join $tlines "\n"]
