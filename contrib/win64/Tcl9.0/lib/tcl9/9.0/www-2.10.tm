...
namespace eval www {
	...
	get post head put delete patch log configure register certify cookiedb
...
proc www::encrypt {sock host} {
    variable tlscfg
    if {[catch {package require tls 2}]} {
	package require tls
    }
	...
	switch -glob $n {
	    ...
	    * {
		...
		if {$x < [string first :// $url] + 3} {
		    ...
		    set url [string replace $url [incr x] end $n]
		}
	    }
	}
    }
...
proc www::filename {path} {
    # RFC7578 section 4.2: The encoding method described in RFC5987, which
    # would add a "filename*" parameter to the Content-Disposition header
    # field, MUST NOT be used.
    set rc ""
    set filename [file tail $path]
    set map {\\ \\\\ \" \\"}
    return [format {; filename="%s"} [encoding convertto utf-8 [string map $map $filename]]]
}
...
proc www::bodypart {data {disp ""}} {
	...
	if {[dict exists $data filename]} {
	    append dispstr [filename [dict get $data filename]]
	} elseif {[dict exists $data file]} {
	    append dispstr [filename [dict get $data file]]
	}
...
proc www::bodybuilder {method url request args} {
    ...
    if {[llength $query]} {
	# Check that the url contains a / after the host part. If not, add it.
...
proc www::request {method url request args} {
	...
	if {$redir} {
	    # A 302 response would officially need confirmation by the user to
	    # send POST requests to a different location. So most existing user
	    # agent implementations treat 302 as if it were a 303 response.
	    if {$code in {302 303}} {
		set method GET
		set request \
		  [dict remove $request body parts partdata multipart]
		# Remove any Content-Length and Content-Type headers
		dict update request headers hdrs {
		    header replace hdrs Content-Length
		    header replace hdrs Content-Type
		}
	    }
...
proc www::patch {args} {
    request PATCH {*}[parseopts [stdopts [postopts]] $args]
}
