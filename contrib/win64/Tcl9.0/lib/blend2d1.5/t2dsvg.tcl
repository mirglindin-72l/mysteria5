...
namespace eval BL::SVG {   
	proc buildBLPathFromSVGPathD {blPath dataStr} {
		...
		while {1} {
			...
			if { $cmd == "" } break
 			 # fix: if previous cmd was a "Z" and current cmd is NOT a M",
 			 #  then Blend2d requires an explicit moveTo()
 			if { [string toupper $prevCmd] eq "Z" } {
				if { [string toupper $cmd] ne "M" } {
					$blPath moveTo $OXY
				}
			}
