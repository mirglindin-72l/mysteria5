...
// in our case
static int tcl_opus_enc (void *clientData,
	Tcl_Interp *interp,
	int objc,
	Tcl_Obj *const objv[]) {
...
// an Opus frame is 40 ms of PCM signal,
// in our case
static int tcl_opus_dec (void *clientData,
	Tcl_Interp *interp,
	int objc,
	Tcl_Obj *const objv[]) {
...
int Opus_wrapper_Init(Tcl_Interp *interp) {
	printf("init TCL stubs for Opus\n");
	if (Tcl_InitStubs(interp, "8.6-", 0) == NULL) {
		return TCL_ERROR;
	}
	/*
	printf("init TCL require for Opus\n");
	if (Tcl_PkgRequire(interp, "Tcl", "8.6-", 0) == NULL) {
		return TCL_ERROR;
	}
	*/
	printf("creating opus commands\n");
	Tcl_CreateObjCommand(interp, "::opus::enc", tcl_opus_enc, NULL, NULL);	
	Tcl_CreateObjCommand(interp, "::opus::dec", tcl_opus_dec, NULL, NULL);	
	return TCL_OK;
}
